---
name: ad-versions
description: Cross-platform ad copy + creative version writer for Filip Kostic — researches the brand's customers (asks first, then scans the website, reviews, Reddit and competitors' sites, reviews and Meta Ad Library when answers are missing), turns one real customer problem into a one-sentence angle, and produces 3 distinct, paste-ready copy versions PLUS matching creative descriptions built on the "Killer Ad Creatives" method (hook → solution → show it → visual proof, one change per variation) and a fair test plan. Use this skill EVERY TIME Filip wants ad variations or new ads for a brand, picks angles/concepts and wants copy, or says "3 versions of ad copy", "ad variations for [brand]", "new creatives for [client]", "copy for angle/concept X", "write the ads for these angles", "Google and Meta copy for", "versions for P1/P2 concepts", "copy + creative for [angle]". Handles Meta (primary text, headline, description, CTA), Google Search (RSA headlines + descriptions), and Google Display (short/long headlines + description). For full standalone RSA builds use rsa-writer; for deep production briefs hand off to creative-brief; for iterating a proven winner use winner-variations.
---
